...
def assign_speakers(
    ...
    empty or no overlap is found, all segments default to ``SPEAKER_00``.
    ...
    for seg in segments:
        # Create a copy so we never mutate the original segment dict
        seg_copy = dict(seg)
 
        seg_start = seg["start"]
        seg_end = seg["end"]
 
        best_speaker = "SPEAKER_00"  # default when no overlap found
        best_overlap = 0.0
 
        for interval in diarization:
            diar_start = interval["start_s"]
            diar_end = interval["end_s"]
 
            # Temporal overlap between segment and diarization interval
